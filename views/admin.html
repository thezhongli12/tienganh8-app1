...
// 2. MODEL NGƯỜI DÙNG (Thêm timestamps để lưu thời gian đăng ký)
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        if (await User.findOne({ username })) return res.json({ success: false, message: "Tên đăng nhập đã tồn tại!" });
...
app.post('/api/login', async (req, res) => {
    const { username, password } = req.body;
    // Kiểm tra Admin (Mật khẩu 080212 của bạn)
    if (password === "080212") {
...
// 8. API QUẢN TRỊ (Hiện mật khẩu và thời gian)
